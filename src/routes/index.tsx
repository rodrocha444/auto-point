...
function RouteComponent() {
  ...
  const { data: todayPoints } = usePointsByDate({
    date: today,
  });
  ...
  const totalAReceber =
    ...
    (totalMsInMonth?.milliseconds / 3600000) *
      parseInt(import.meta.env.VITE_VALOR_POR_HORA || "0");
    ...
    <div className="overflow-hidden h-full w-full items-center p-5 gap-5 flex flex-col">
      ...
      <div className="flex flex-col gap-5 w-full flex-1 justify-center">
            ...
            {todayPoints?.map(point => (
              <div key={point.id} className="flex gap-2 items-center">
                ...
                <Trash
                  ...
                  size="20"
                  className="text-red-400 active:text-red-300 cursor-pointer"
                />
              </div>
            ))}
        ...
        <div className="text-white flex flex-col items-center bg-gray-800 w-full rounded-lg p-4">
          ...
          {!!todayPoints?.length && todayPoints.length % 2 === 1 && (
        ...
        <div className="flex flex-col gap-2 w-full">
          <button
            className="bg-gray-400 w-full rounded-lg p-3 active:bg-gray-200 transition-colors flex justify-center cursor-pointer"
