...
import {
  ...
  usePointsInIntervalWithTotalQuery,
...
const datesSchema = z
  ...
  .superRefine(({ startDate, endDate }, ctx) => {
    // Se algum for nulo, paramos aqui para não quebrar a lógica de dias abaixo
    if (!startDate || !endDate) return;

    // 2. Validação de Intervalo Negativo
    if (endDate < startDate) {
      ctx.addIssue({
        code: "custom", // Usei 'custom' para simplificar
...
function RouteComponent() {
  ...
  const { data: totalMsData } = usePointsInIntervalWithTotalQuery(
    {
      input: {
  ...
  const totalAReceber =
    totalMsData?.pointsInIntervalWithTotal.milliseconds &&
    (totalMsData?.pointsInIntervalWithTotal.milliseconds / 3600000) *
  ...
  return (
    <div className="p-5 flex flex-col gap-5 items-center overflow-auto h-dvh">
      ...
      <div className="flex items-stretch gap-2">
        <button
          className="py-2 bg-gray-600 text-white rounded-md font-medium px-10"
      ...
      {totalMsData && (
        <div className="text-white flex flex-col items-center gap-5">
          ...
          <div className="flex flex-col items-center">
            ...
            {totalMsData.pointsInIntervalWithTotal.points.map(point => (
