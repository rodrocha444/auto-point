...
function RouteComponent() {
  ...
  const { data: points, refetch: refetchPoints } = usePointsByDateQuery({
    date: format(date, "yyyy-MM-dd"),
  });

  const { mutate: deletePoint, isPending: isDeleting } = useDeletePointMutation(
    {
  ...
  return (
    <div className="p-5 flex flex-col gap-6 items-center">
      ...
      {points?.pointsByDate && points?.pointsByDate.length > 0 ? (
